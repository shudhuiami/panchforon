...
/** One day of the plan: a card on a phone, a column in the desktop grid. */
export const PlanDayColumn: React.FC<PlanDayColumnProps> = ({ iso, items, days, onAddDish, readOnly = false }) => {
    ...
    return (
        <section
            ...
            className={`flex flex-col rounded-3xl border p-4 ${today ? 'border-primary/50 bg-primary-soft/30' : past ? 'border-line bg-surface/60' : 'border-line bg-surface'}`}
        >
            ...
            {dishes === 0 ? (
                <p className="mt-4 rounded-2xl border border-dashed border-line-strong px-3 py-5 text-center text-xs text-ink-3">
                    {readOnly ? 'Nothing was cooked this day.' : 'Nothing planned yet.'}
                </p>
            ) : (
